...
with another element that has the same value, except for one element that is left
unpaired.
...
function solution($A)
{
    ...
    foreach ($A as $integer) {
        if (!isset($oddOccurences[$integer])) {
            // If there is no integer until now
            $oddOccurences[$integer] = 1;
        } else {
            // Remove Pair
            unset($oddOccurences[$integer]);
        }
    }

    return key($oddOccurences);
}

var_dump(solution([9, 3, 3, 9, 7, 9]));//ans is 7